...
void nd_init_log(void) {
	...
	if(enable_syslog) openlog("newsdist", LOG_PID, LOG_FACILITY);
#endif
	nd_log_info("NewsDist/" NEWSDIST_VERSION " starting up");
}

void nd_log_string(char* out) {
	time_t	   t  = time(NULL);
...
#ifdef USE_SYSTEM_SYSLOG
	if(enable_syslog) syslog(LOG_INFO, info);
#endif
...
#ifdef USE_SYSTEM_SYSLOG
	if(enable_syslog) syslog(LOG_NOTICE_LEVEL, info);
#endif
	if(enable_stderr_log) {
		nd_log_string(timestr);
