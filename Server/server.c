...
#ifdef HAS_SSL
#	ifdef HAS_IPV4
	if(ssl_key != NULL && ssl_cert != NULL) server_sockets[SECURE] = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
#	endif
#	ifdef HAS_IPV6
	if(ssl_key != NULL && ssl_cert != NULL) server_sockets[SECURE6] = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
#	endif
#endif
...
int nd_loop_server(void) {
	...
	for(i = 0; i < sizeof(server_sockets) / sizeof(server_sockets[0]); i++) {
		if(server_sockets[i] != NO_SOCKET) count++;
	}
	...
	while(1) {
		...
		if(n < 0) {
			...
			for(i = 0; i < sizeof(server_sockets) / sizeof(server_sockets[0]); i++) {
				...
				if(server_sockets[i] == NO_SOCKET) continue;
#if defined(HAS_POLL)
				if(fds[incr].revents & POLLIN) go = 1;
#elif defined(HAS_SELECT)
				if(FD_ISSET(server_sockets[i], &fdset)) go = 1;
#endif
				if(go) {
					int sock;
